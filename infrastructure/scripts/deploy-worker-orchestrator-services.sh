...
# VoiceCode Worker and Orchestrator Services Deployment
# Phase 2 deployment script for MCP-enabled services
...
EOF
    ...
    case $SERVICE_NAME in
      ...
      - name: ServiceEndpoints__WorkerService
        value: https://${PROJECT_PREFIX}-worker.${LOCATION}.azurecontainer.io
      - name: WorkerManagement__MaxConcurrentWorkers
        value: '5'
      - name: WorkerManagement__WorkerQueueName
...
EOF
      ...
      - name: ClaudeCode__MaxConcurrentWorkers
        value: '5'
      - name: ClaudeCode__WorkerTimeout
        value: '00:30:00'
...
EOF
    ...
    # Add Azure Files volume if requested
    if [ "$SERVICE_NAME" = "worker" ] && [ "$USE_AZURE_FILES" = "true" ]; then
        # Extract storage account name and key from connection string
        STORAGE_ACCOUNT=$(echo $STORAGE_CONN | sed -n 's/.*AccountName=\([^;]*\).*/\1/p')
        STORAGE_KEY=$(echo $STORAGE_CONN | sed -n 's/.*AccountKey=\([^;]*\).*/\1/p')
        
        cat >> "${SERVICE_NAME}-container.yaml" << EOF
  volumes:
  - name: workspaces
    azureFile:
      shareName: worker-workspaces
...
# Function to create Azure Files share
create_azure_files_share() {
    echo "Setting up Azure Files share for worker workspaces..."
    ...
    if [ -z "$STORAGE_ACCOUNT" ]; then
        echo "ERROR: Could not extract storage account name from connection string"
        return 1
    fi
    
    # Check if share exists
    SHARE_EXISTS=$(az storage share exists --name worker-workspaces --account-name $STORAGE_ACCOUNT --query exists -o tsv 2>/dev/null || echo "false")
    
    if [ "$SHARE_EXISTS" = "false" ]; then
        echo "Creating Azure Files share 'worker-workspaces'..."
        az storage share create \
            --name worker-workspaces \
            --account-name $STORAGE_ACCOUNT \
            --quota 100 \
            --output none
        echo "✓ Azure Files share created"
    else
        echo "✓ Azure Files share 'worker-workspaces' already exists"
    fi
}

# Main deployment
echo "========================================="
echo "VoiceCode Worker and Orchestrator Services Deployment"
echo "========================================="
...
# Ask about Azure Files
echo ""
read -p "Do you want to set up persistent workspace storage with Azure Files? (y/n): " USE_AZURE_FILES
if [ "$USE_AZURE_FILES" = "y" ]; then
    create_azure_files_share
...
# Deploy services
deploy_service_with_identity "orchestrator" "0.5" "1.0" "$LATEST_TAG" "false"
deploy_service_with_identity "worker" "1.0" "2.0" "$LATEST_TAG" "$USE_AZURE_FILES"

echo "========================================="
echo "Worker and Orchestrator services deployed successfully!"
echo "========================================="

# Show service URLs
echo ""
echo "Service URLs:"
echo "Orchestrator: https://${PROJECT_PREFIX}-orchestrator.${LOCATION}.azurecontainer.io"
echo "Worker: https://${PROJECT_PREFIX}-worker.${LOCATION}.azurecontainer.io"

# Update dispatcher service
echo ""
echo "IMPORTANT: You need to update the dispatcher service to include the new endpoints."
echo "Run the following command to update the dispatcher:"
echo ""
echo "az container create --resource-group $RESOURCE_GROUP --name ${PROJECT_PREFIX}-dispatcher-ci \\"
echo "  --environment-variables \\"
echo "    ServiceEndpoints__OrchestratorService=https://${PROJECT_PREFIX}-orchestrator.${LOCATION}.azurecontainer.io \\"
echo "    ServiceEndpoints__WorkerService=https://${PROJECT_PREFIX}-worker.${LOCATION}.azurecontainer.io"
