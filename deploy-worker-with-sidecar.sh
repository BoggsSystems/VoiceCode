...
# Deploy Worker Service with Claude SDK Sidecar to Azure Container Apps
# This creates a multi-container app with both services
...
WORKER_NAME="voicecode-worker-sidecar"
WORKER_ID="worker-sidecar-1"

echo -e "${BLUE}Worker Service with Sidecar Deployment${NC}"
echo -e "${BLUE}=====================================${NC}"
...
# Create Container App with multiple containers
echo -e "\n${YELLOW}Creating multi-container app...${NC}"

# Create YAML configuration for multi-container deployment
cat > worker-sidecar-config.yaml << EOF
location: East US
name: $WORKER_NAME
      ...
      - name: worker
        ...
        env:
          ...
          - name: Worker__EnableStreaming
            value: "true"
      - name: sidecar
        image: $REGISTRY_NAME.azurecr.io/claude-sdk-sidecar:latest
        resources:
          cpu: 0.5
          memory: 1Gi
        env:
...
az containerapp create \
    ...
    --yaml worker-sidecar-config.yaml

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Worker with sidecar created successfully${NC}"
else
    echo -e "${RED}✗ Failed to create worker with sidecar${NC}"
    exit 1
fi
...
# Clean up
rm -f worker-sidecar-config.yaml

echo -e "\n${GREEN}========================================${NC}"
echo -e "${GREEN}Worker + Sidecar Deployment Complete${NC}"
echo -e "${GREEN}========================================${NC}"
echo -e "Worker URL: ${BLUE}https://$WORKER_URL${NC}"
echo -e "\nThe worker service is now running with the Claude SDK sidecar!"
echo -e "\nTo test the sidecar integration:"
echo -e "1. Check worker health: ${YELLOW}curl https://$WORKER_URL/health${NC}"
echo -e "2. Check sidecar (internal): The sidecar is only accessible from the worker container"
echo -e "3. Monitor logs: ${YELLOW}az containerapp logs tail -n $WORKER_NAME -g $RESOURCE_GROUP --follow${NC}"